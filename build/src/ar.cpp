...
*/
    ...
    public:
        ...
        ar_result run() override
        {
            ...
            {
                throw std::runtime_error("--ar-dir is not implemented yet.");
            }
    ...
    ar::mapping map_nodes(const phylo_tree& extended_tree, const phylo_tree& ar_tree)
    {
        ...
        for (const auto& ext_node : visit_subtree<const_iterator>(extended_tree.get_root()))
        {
            ...
            if (ext_node.is_leaf())
            {
                ...
                //std::cout << "Leaf Map " << label << " -> " << ext_to_ar[label] << std::endl;
                ...
                if (!ext_node.is_root())
                {
                    ...
                    //std::cout << "Parent Map " << ext_parent_ptr->get_label() << " -> " << ext_to_ar[ext_parent_ptr->get_label()] << std::endl;
                }
            }
            /// Internal node
            else
            {
                ...
                {
                    ext_to_ar[ext_parent_ptr->get_label()] = ar_parent_ptr->get_label();
                    //std::cout << "Internal Map " << ext_parent_ptr->get_label() << " -> " << ext_to_ar[ext_parent_ptr->get_label()] << std::endl;
                }
            }
        }
