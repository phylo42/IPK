...
*/
    ...
    std::tuple<proba_matrix, xcl::phylo_tree> ancestral_reconstruction(ar::software software, const ar::parameters& parameters)
    {
        ...
        auto ar_tree = xcl::io::load_newick(result.tree_file);

        return { std::move(matrix), std::move(ar_tree) };
    }

    size_t count_leaves(const xcl::phylo_tree& tree)
    {
        size_t count = 0;
    ...
    ar::mapping map_nodes(const xcl::phylo_tree& extended_tree, const xcl::phylo_tree& ar_tree)
    {
        ...
        using const_iterator = xcl::postorder_tree_iterator<true>;
        for (const auto& ext_node : xcl::visit_subtree<const_iterator>(extended_tree.get_root()))
        {
            if (ext_node.is_root())
            {
                continue;
