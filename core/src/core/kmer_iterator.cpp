    ...
    bool> kmer_iterator<no_ambiguity_policy>::encode_kmer()
{
    ...
    else
    {
        ...
        if (auto code = core::encode_kmer<no_ambiguity_policy>(kmer_view); code)
        {
            key = *code;
            success = true;
        }
    ...
    bool> kmer_iterator<no_ambiguity_policy>::encode_kmer_from_previous() const
{
    ...
    if (position <= _sequence_view.size() - _kmer_size)
    {
        ...
        if (auto new_code = core::encode<no_ambiguity_policy>(new_base); new_code)
        {
    ...
    bool> kmer_iterator<one_ambiguity_policy>::encode_kmer()
{
    ...
    else
    {
        ...
        /// resolve ambiguities
        if (auto codes = core::encode_kmer<one_ambiguity_policy>(kmer_view); codes)
        {
            keys = *codes;
            success = true;
        }
    ...
    bool> kmer_iterator<one_ambiguity_policy>::encode_kmer_from_previous() const
{
    ...
    if (position <= _sequence_view.size() - _kmer_size)
    {
        ...
        auto new_codes = core::encode<one_ambiguity_policy>(new_base);
        ...
        {
            auto suffix1 = keys[0];
            suffix1 <<= core::bit_length<core::seq_type>();
            suffix1 &= ((1u << (core::bit_length<core::seq_type>() * _kmer_size)) - 1);

            auto suffix2 = keys[1];
            suffix2 <<= core::bit_length<core::seq_type>();
            suffix2 &= ((1u << (core::bit_length<core::seq_type>() * _kmer_size)) - 1);
        ...
        if (new_codes && !more_than_1_ambiguity)
        {
            ...
            else
            {
                ...
                for (const auto new_code : *new_codes)
                {
                    /// shift bits and remove the first base of the current k-mer
                    auto new_key = key;
                    new_key <<= core::bit_length<core::seq_type>();
                    new_key &= ((1u << (core::bit_length<core::seq_type>() * _kmer_size)) - 1);
